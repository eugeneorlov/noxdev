...
### Added
- `noxdev demo` now validates `uv` and `python3` requirements at start with clear error messages

### Fixed
- Cost tracking: fixed path encoding in `findLatestSessionFile` function
- Cost tracking: added `~/.claude` mount to `docker-run-api.sh` for proper session access
- Dashboard: tsup config now preserves `node:` prefix on built-in imports

### Changed
- Documentation: added `uv` as required dependency in README
