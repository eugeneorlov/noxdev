...
#   $10  api_key           Gemini API key

set -euo pipefail

if [ "$#" -ne 10 ]; then
  echo "Error: expected 10 arguments, got $#" >&2
  echo "Usage: docker-run-gemini.sh <prompt_file> <task_log> <timeout_seconds> <worktree_dir> <project_git_dir> <git_target_path> <memory_limit> <cpu_limit> <docker_image> <api_key>" >&2
  exit 1
fi
