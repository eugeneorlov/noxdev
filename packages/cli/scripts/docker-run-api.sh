...
if [ "$#" -ne 11 ] && [ "$#" -ne 12 ]; then
  echo "Error: expected 11 or 12 arguments, got $#" >&2
  echo "Usage: docker-run-api.sh <prompt_file> <task_log> <timeout_seconds> <worktree_dir> <project_git_dir> <git_target_path> <memory_limit> <cpu_limit> <docker_image> <task_log_dir> <api_key> [model]" >&2
  exit 1
fi
